...
These handlers listen to events and update the read models (MongoDB collections).
 This keeps your existing Mongoose models in sync with the event store.
 */
...
export async function createUserProjectionHandler() {
  ...
  userProjection.handle("user", "user-created", async (id: string, evt: Extract<UserEvt, { type: 'user-created' }>) => {
    const hashedPassword = await bcrypt.hash(evt.password, 10);
  ...
  userProjection.handle("user", "user-created-oauth", async (id: string, evt: Extract<UserEvt, { type: 'user-created-oauth' }>) => {
    const profileData =
      evt.provider === "google"
        ? {
  ...
  userProjection.handle("user", "user-verified", async (id: string, _evt: Extract<UserEvt, { type: 'user-verified' }>) => {
  ...
  userProjection.handle("user", "profile-updated", async (id: string, evt: Extract<UserEvt, { type: 'profile-updated' }>) => {
    await User.findByIdAndUpdate(id, {
  ...
  userProjection.handle("user", "wallet-linked", async (id: string, evt: Extract<UserEvt, { type: 'wallet-linked' }>) => {
    await User.findByIdAndUpdate(id, {
  ...
  userProjection.handle("user", "kyc-submitted", async (id: string, _evt: Extract<UserEvt, { type: 'kyc-submitted' }>) => {
    await User.findByIdAndUpdate(id, {
  ...
  // Handle KYC approval
  userProjection.handle("user", "kyc-approved", async (id: string, _evt: Extract<UserEvt, { type: 'kyc-approved' }>) => {
    await User.findByIdAndUpdate(id, {
      kycVerified: true,
      updatedAt: new Date(),
    });
  ...
  userProjection.handle("user", "kyc-rejected", async (id: string, _evt: Extract<UserEvt, { type: 'kyc-rejected' }>) => {
    await User.findByIdAndUpdate(id, {
      kycVerified: false,
      updatedAt: new Date(),
    });
  ...
  userProjection.handle("user", "referral-used", async (id: string, evt: Extract<UserEvt, { type: 'referral-used' }>) => {
    await User.findByIdAndUpdate(id, {
  ...
  userProjection.handle("user", "referral-earned", async (id: string, evt: Extract<UserEvt, { type: 'referral-earned' }>) => {
    await User.findByIdAndUpdate(id, {
  ...
  // Handle user ban
  userProjection.handle("user", "user-banned", async (id: string, evt: Extract<UserEvt, { type: 'user-banned' }>) => {
    await User.findByIdAndUpdate(id, {
      isBanned: true,
  ...
  // Handle user unban
  userProjection.handle("user", "user-unbanned", async (id: string, _evt: Extract<UserEvt, { type: 'user-unbanned' }>) => {
    await User.findByIdAndUpdate(id, {
      isBanned: false,
      banReason: undefined,
      isActive: true,
  ...
  // Handle role change
  userProjection.handle("user", "role-changed", async (id: string, evt: Extract<UserEvt, { type: 'role-changed' }>) => {
    await User.findByIdAndUpdate(id, {
      role: evt.newRole,
      updatedAt: new Date(),
    });
  ...
  userProjection.handle("user", "user-deactivated", async (id: string, _evt: Extract<UserEvt, { type: 'user-deactivated' }>) => {
    await User.findByIdAndUpdate(id, {
      isActive: false,
      updatedAt: new Date(),
    });
  ...
  userProjection.handle("user", "user-reactivated", async (id: string, _evt: Extract<UserEvt, { type: 'user-reactivated' }>) => {
    await User.findByIdAndUpdate(id, { isActive: true, updatedAt: new Date() });
    console.log(`▶️ User reactivated: ${id}`);
  });
...
export async function createSavingsProjectionHandler() {
  ...
  savingsProjection.handle("savings", "plan-created", async (id: string, evt: Extract<SavingsEvt, { type: 'plan-created' }>) => {
  ...
  savingsProjection.handle("savings", "deposit-confirmed", async (id: string, evt: Extract<SavingsEvt, { type: 'deposit-confirmed' }>) => {
  ...
  savingsProjection.handle("savings", "interest-accrued", async (id: string, evt: Extract<SavingsEvt, { type: 'interest-accrued' }>) => {
    await SavingsPlan.findByIdAndUpdate(id, {
  ...
  savingsProjection.handle(
    ...
    async (id: string, evt: Extract<SavingsEvt, { type: 'withdrawal-completed' }>) => {
      await SavingsPlan.findByIdAndUpdate(id, {
        $push: {
          withdrawals: {
            amount: evt.amount,
  ...
  savingsProjection.handle("savings", "plan-completed", async (id: string, evt: Extract<SavingsEvt, { type: 'plan-completed' }>) => {
    await SavingsPlan.findByIdAndUpdate(id, {
      status: "completed",
      endDate: evt.endDate,
  ...
  // Handle plan penalty
  savingsProjection.handle("savings", "plan-penalized", async (id: string, evt: Extract<SavingsEvt, { type: 'plan-penalized' }>) => {
    await SavingsPlan.findByIdAndUpdate(id, {
      status: "penalized",
      earlyWithdrawalPenalty: evt.penaltyAmount,
      updatedAt: new Date(),
    });
    console.log(`⚠️ Plan penalized: ${id} -> $${evt.penaltyAmount}`);
  });

  // Handle plan closure
  savingsProjection.handle("savings", "plan-closed", async (id: string, _evt: Extract<SavingsEvt, { type: 'plan-closed' }>) => {
    await SavingsPlan.findByIdAndUpdate(id, {
      status: "withdrawn",
      updatedAt: new Date(),
    });
